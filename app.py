...
bard_client = Bard(token=os.environ.get("PALM_API_KEY"))
# Initialize the app
app = Flask(__name__)
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://flask_user:StrongPassword123!@localhost/travel_app_db'
db = SQLAlchemy(app)
app.secret_key = secret_key
...
def index():
    ...
    if request.method == "POST":
        ...
        try:
            prompt = f"Generate a detailed travel itinerary from {source} to {destination} starting on {start_date} and ending on {end_date} for {no_of_day} days. Include places to visit, activities, and travel tips."
            response = bard_client.get_answer(prompt)
